function [opdata,x,iter,fgmresits,flag]= fgmreswlsir_qr(A,b,precf,precw,precr,iter_max,gtol)
% fgmreswlsir_qr:  FGMRES-based iterative refinement solver for WLS 
% problems in three precisions with left QR preconditioner.
...
%       * double if precr = 2,
%       * quad if precr = 4
%     Uses split preconditioned FGMRES M\A with left QR preconditioner
...
% Construct left preconditioner in precw, composed of R
% factors computed in precf
if precw == 1
    L1 = single([alpha.*inv(D), single(Q1*R); single(R'*Q1'), zeros(n)]);
elseif precw == 2
    L1 = double([alpha.*inv(D), double(Q1*R); double(R'*Q1'), zeros(n)]);
else
    L1 = chop([alpha.*inv(D), hgemm(Q1,R); hgemm(R',Q1'), zeros(n)]);
end

% Form "exact" preconditioned system via Advanpix
extPC = double(L1)\double(Aug_A);
...
while ~cged
    ...
    if precw == 2
        [d, err, its, flag] = fgmres_dq( Aug_A, zeros(m+n,1), Aug_rhs/nrhs, L1, eye(m+n), m+n, 1, gtol);
    else
        [d, err, its, flag] = fgmres_sd( Aug_A, zeros(m+n,1), Aug_rhs/nrhs, L1, eye(m+n), m+n, 1, gtol);
