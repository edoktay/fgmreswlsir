function [x, error, its, flag] = fgmres_sd( A, x, b, L1, R1, restrt, max_it, tol)
...
for iter = 1:max_it                             
    ...
    % Apply left preconditioner to vector in quad precision
    r = double(L1)\double(rtmp);
    ...
    for i = 1:m                                                             % construct orthonormal basis via GS
        ...
        % Apply right preconditioner to vector in quad precision
        vcur = double(R1)\double(vcur);
        Z(:,i) = single(vcur);      

        % Apply matrix and left preconditioner to vector in quad precision
        vcur = double(L1)\(double(A)*double(vcur));
    ...
    % Apply left preconditioner to vector in quad precision
    r = double(L1)\double(rtmp);                                            % compute residual
    % Store result in double precision
    r = single(r);
